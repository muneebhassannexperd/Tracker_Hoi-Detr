...
def is_machine_scale_bbox(
    ...
    """Reject whole-machine / door-strip FO (huge area or ultra-wide)."""
    w, h = _bbox_wh(box)
    if w < 1.0 or h < 1.0:
        return True
    area = w * h
    ...
    aspect = w / h
    if area / frame_area >= MACHINE_FO_MAX_AREA_FRAC:
        return True
    if w / max(frame_w, 1.0) >= MACHINE_FO_MAX_WIDTH_FRAC:
        return True
    if aspect >= MACHINE_FO_MAX_ASPECT:
        return True
    return False
...
# Do not continue a shop hand if it hops ABOVE the outer ROI top (upper
# door / machine header / background). Leave room only for lateral/bottom
# carry exits (customer side is typically +y past the opening).
HAND_ABOVE_ROI_MARGIN_PX: float = 8.0
...
def filter_roi_detections(
    ...
    - firstobject / products: centroid must be inside outer ROI (plus machine filter
      already applied).
    - hands: *start* only inside ROI; continue outside only if overlapping a hand
      box already accepted last frame (shop hand that left the ROI) AND not above
      the ROI top (upper-door / non-shop zones).
    """
    ...
    for d in detections:
        ...
        # Strictly above outer ROI top → never track (door/header/bystander).
        cy = float(d.centroid[1]) if hasattr(d.centroid, "__len__") else float(d.centroid)
        if cy < top_y - HAND_ABOVE_ROI_MARGIN_PX:
            continue

        # Outside ROI (sides/bottom): only continue tracks that already entered.
...
PUTBACK_MIN_AFTER_PICKUP: int = 90      # block place during multi-item pick burst
PUTBACK_MIN_PLACE_AFTER_PICKUP: int = 80  # place stamp itself must be after last pick
...
class PickupEngineV2(PickupPutbackEngine):
    ...
    def process_frame(
        ...
        for hand_track_id, object_track_id, link_conf in resolve_hf_links_to_tracks(
            ...
            # Shelf interaction only inside outer ROI. Hands tracked outside after
            # ROI birth may still exist for ID, but door / ambient HF must not
            # drive contact / regrab (a774 ~f312 upper-door touch).
        ...
        for hand_obs in observations:
            ...
            if allow_contact:
                ...
                if live_ok:
                    ...
                    # place without live object box (common on put-downs)
                    # Blocked in the door band so door-handle grip ≠ putback.
                    contact_ok = True
